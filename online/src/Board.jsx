...
// 行動メニューのボタン（アイコン上・ラベル下）
function MenuBtn({ icon, label, sub, tip, disabled, onClick, accent }) {
  return (
    <Tip text={tip}>
      <button className={`menu-btn${accent ? ' menu-btn--accent' : ''}`} disabled={disabled} onClick={onClick}>
        <span className="mb-ico">{icon}</span>
        <span className="mb-lb">{label}{sub ? <small>{sub}</small> : null}</span>
      </button>
    </Tip>
  );
}
  ...
  if (hand.length === 0) {
    return (
      <div className="hand-section">
        <p className="hand-title">🃏 自分の手札：なし（秋冬に「カードを引く」で入手）</p>
      </div>
    );
  }

  // 同種カードをまとめる（card.id 単位・代表 handIdx を保持）
  const groups = [];
  hand.forEach((card, idx) => {
    let g = groups.find((x) => x.id === card.id);
    if (!g) { g = { id: card.id, card, indices: [] }; groups.push(g); }
    g.indices.push(idx);
  });

  // 対象選択画面（成長肥料 / 品質肥料）
  if (pick) {
    const isQ = pick.cardId === 'quality_fert';
    const targets = isQ
    ...
    return (
      <div className="hand-section">
        ...
        <div className="hand-cards">
          ...
          {targets.length === 0 && <span style={{ fontSize: 12, color: '#888' }}>対象の田なし（育成中の田が必要）</span>}
        </div>
        <button className="back" onClick={() => setPick(null)}>← 戻る</button>
      </div>
    );
  }
        ...
        <div className="tt-field">
          {f.status === 'empty' ? <span className="tt-empty">{action === 'plant' ? '🌱植' : '空'}</span>
            : <>
              ...
              <span className="tt-w" style={{ color: WATER_COLOR[f.water] }}>💧{f.water}</span>
            </>}
        </div>
      ) : (
